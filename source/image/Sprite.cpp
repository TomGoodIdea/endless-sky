...
namespace {
	void AddBuffer(ImageBuffer &buffer, uint32_t *target, bool noReduction)
	{
		...
		if(!noReduction
				&& ((setting == Preferences::LargeGraphicsReduction::LARGE_ONLY && buffer.Width() * buffer.Height() >= 1000000)
				|| setting == Preferences::LargeGraphicsReduction::ALL
				|| setting == Preferences::LargeGraphicsReduction::ALL_X16))
			buffer.Shrink(setting == Preferences::LargeGraphicsReduction::ALL_X16 ? 4 : 2);
		...
		int filter = setting == Preferences::LargeGraphicsReduction::ALL_X16 ? GL_NEAREST : GL_LINEAR;
...
void Sprite::AddFrames(ImageBuffer &buffer, bool is2x)
{
	...
	if(buffer.Pixels())
		AddBuffer(buffer, &texture[is2x], name.starts_with("ui/") || name.starts_with("_menu/"));
}
...
void Sprite::AddSwizzleMaskFrames(ImageBuffer &buffer, bool is2x)
{
	...
	AddBuffer(buffer, &swizzleMask[is2x], name.starts_with("ui/") || name.starts_with("_menu/"));
}
